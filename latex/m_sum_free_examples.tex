...
\[
    \alpha\leq \frac1{m+2}.
\]
The examples below exceed this bound for \(m=4\) and \(m=5\). We also record
a general equally spaced construction which gives examples of density larger
than \(1/8\) for every \(m\geq 3\).
...
Let \(m\geq 3\), and let \(q\) be an integer satisfying
\[
    1\leq q < \frac{m+2}{2}.
\]
Set
...
\[
    ...
    \subset \mathbb T.
\]
Then \(A_{m,q}\) is \(m\)-sum-free and
\[
    \mu(A_{m,q})
    =
    \frac{q(m-2q+2)}{m(m+2)}.
\]
In particular, optimizing over \(q\) gives the lower bound
\[
    \sup \{\mu(A): A\subset\mathbb T \text{ is } m\text{-sum-free}\}
    \geq
...
\begin{proof}
The intervals are disjoint because \(\delta<1/m\). Write
\[
    I_r=\left(a+\frac r m,\; a+\frac r m+\delta\right),
    \qquad 0\leq r\leq q-1.
\]
For \(0\leq r,s,t\leq q-1\), the interval \(I_r+I_s-mI_t\) has endpoints
\begin{align*}
    L_{rst}
    &=
    (2-m)a+\frac{r+s}{m}-t-m\delta, \\
    R_{rst}
    &=
    (2-m)a+\frac{r+s}{m}-t+2\delta.
\end{align*}
By the choice of \(a\), there is an integer \(h\) such that
\[
    (2-m)a=h+m\delta.
\]
Thus
\[
    L_{rst}=h-t+\frac{r+s}{m}
\]
and
\[
    R_{rst}
    =
    h-t+\frac{r+s}{m}+(m+2)\delta
    =
    h-t+\frac{r+s}{m}+1-\frac{2q-2}{m}.
\]
Since \(0\leq r+s\leq 2q-2\), we have
\[
    h-t
    \leq L_{rst}
    <
    ...
    \leq h-t+1.
\]
Therefore each open interval \(I_r+I_s-mI_t\) contains no integer. Hence
\(0\notin A_{m,q}+A_{m,q}-mA_{m,q}\) in \(\mathbb T\).

The measure is \(q\delta\). It remains only to maximize
\[
    q(m-2q+2)=q((m+2)-2q)
\]
over integers \(q\). This quadratic has maximum
\[
    \left\lfloor \frac{(m+2)^2}{8}\right\rfloor.
\]
Writing \(u=m+2\), we also have
\[
