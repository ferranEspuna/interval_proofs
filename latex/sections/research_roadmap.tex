...
\subsection{Immediate validation and consolidation}

\begin{enumerate}
    \item Read the two open-interval endpoint proofs in
    Sections~\ref{sec:open-two-interval-r5} and~\ref{sec:open-two-interval-r6}
    line by line.  They currently provide two checks of the same classification,
    but the project notes record that they were initially AI-assisted.
    \item Reproduce the full \(m=3\), \(N\leq8\) computation with a fixed
    environment.  Commit small JSON certificates and attach large logs to a
    tagged release.
    \item Regenerate the maximal removable families for \(N=2,3,4,5\), and
    include a machine-readable summary plus the command used for every row.
\end{enumerate}
