...
need to be archived. \\
Omitted interval interactions & \statuscomputed & The maximal-family lists are
exhaustive for \(N=2,3\).  For \(N=4\) the archived list is capped, and for
\(N=5\) all singleton exclusions were tested (32 were removable), but the
bounded family search found no maximal family before timing out. \\
...
\(m=8,9\) improved lower bounds & \statusproved & Exact rational certificates
give densities \(49/320>3/20\) and \(136/891>5/33\), respectively; see
Section~\ref{sec:explicit-constructions}. \\
\(m=12,13\) improved lower bounds & \statusproved & Exact certificates give
densities \(251/1728>1/7\) using seven intervals and
\(122/845>28/195\) using six, respectively; see
Section~\ref{sec:explicit-constructions}. \\
...
Tiny added intervals & \statuscomputed & Complete one- and two-extra local
scans through \(m=100\), followed by saturation tests, expose exact
doubled-tail families with linearly many components and aggregate
\(O(m^{-2})\) gain.  More than \(2.28\) million lifted images were checked
exactly; \(m=12\) is exceptional.  See Section~\ref{sec:extra-intervals}. \\
