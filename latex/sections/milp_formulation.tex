...
Conversely, once an explicit construction of measure \(c\) is known, the
optional cutoff
\[
    \sum_i\alpha_i\geq c
\]
may discard branches that cannot improve on it.  This does not prove the
lower bound and does not change the optimum: its use is justified only by the
independent construction.

\subsection{Scale, visualization, and state sharing}
...
unlikely to scale: every term \(I_i+I_j-mI_\ell\) has a choice of integer lift,
...
is a companion tool.  Its circle mode displays a finite union of intervals
together with the pieces of \(A+A-mA\), and its discrete mode displays the
Candela--Gonz\'alez-S\'anchez--Grynkiewicz construction.  Both modes accept
...
The campaign in \code{json_results/verification_20260716/} records the exact
commands, serialized problems and solutions, solver diagnostics, and compact
summaries for the computations quoted in this document.  Large deterministic
problem files from the \(m\leq100\) scan are stored with gzip compression.

There are three possible mathematical outputs of the interval program:
\begin{enumerate}
...
\end{enumerate}
The current implementation is strongest as the first two.

HiGHS has separate relative and absolute MIP-gap stopping rules, so requesting
zero relative gap alone does not disable its default absolute tolerance.  Even
when both are set to zero, the result remains a floating-point solver
